...
export default function DeleteBudgetModal() {
  ...
  const searchParams = useSearchParams() //Obtener los parametros de la url 
  const deleteBudgetId = searchParams.get('deleteBudgetId')
  const show= deleteBudgetId ? true : false

  const hideModal = new URLSearchParams(searchParams.toString()) // Ocultar modal eliminando los parametros
  hideModal.delete('deleteBudgetId')

  const pathname = usePathname() //Obtiene la ruta actual 
  console.log(pathname)

  const router = useRouter()//replace para eliminar el parametro de la ruta principal
